...
const Users = () => {
  ...
  useEffect(() => {
    fetchedUsers.map(async (user) =>
      fetchBus(user.busId, localStorage.getItem("user"))
        .then((bus) => {
          setRows((prevUsers) => [
            ...prevUsers,
            {
  ...
  const handleCreateUser = () => {
    if ((emailCreate, passwordCreate, roleCreate, usernameCreate, addBusCr)) {
      ...
      if (roleCreate === "admin" && localStorage.getItem("role") === "bus") {
  ...
  const handleSave = () => {
    ...
    if (roleUpdate === "admin" || roleUpdate === "bus") {
      if (roleUpdate === "admin" && localStorage.getItem("role") === "bus") {
